...
class CreateC1App {
  ...
  async main(): Promise<void> {
    try {
      ...
      this.nonInteractive = isNonInteractiveEnvironment(
      ...
      logger.error(
        `Create C1 App failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      process.exit(1);
    }
  }
  ...
  private async parseArguments(): Promise<CLIOptions> {
    const argv = await yargs(hideBin(process.argv))
      ...
      .option("project-name", {
        ...
        type: "string",
        description:
          "Name of the project to create (alternative to positional argument)",
      })
      .option("template", {
        alias: "t",
        type: "string",
    ...
    const apiKey = await input({
      ...
      transformer: (input: string) => {
        ...
        return input.length > 8
          ? `${input.substring(0, 8)}${"*".repeat(Math.min(input.length - 8, 32))}`
          : input;
      },
    });
  ...
  private async authenticateAndGenerateAPIKey(): Promise<AuthenticationResult> {
    ...
    if (!initResult.success) {
      throw new Error(
        initResult.error || "Failed to initialize authentication",
      );
    }
    ...
    const orgId = (userInfoResponse["org_claims"] as { orgId: string }[])?.[0]
      ?.orgId;
  ...
  private async gatherProjectConfig(options: CLIOptions): Promise<void> {
    ...
    logger.success(
      `Project "${this.config.projectName}" will be created with:`,
    );
